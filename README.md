...
- **Lore & rules:** while running `npm run dev`, open `/editor` for a rich, Notion-style editor over `docs/lore/` and `docs/rules/`. Pick a page on the left to edit it, or use **+** to create a new one — creating a page writes the `.md` file and wires it into that section's `index.md` list and sidebar nav (in `docs/.vitepress/config.js`) automatically. Deleting a lore page that still has a map pin pointing at it is blocked until you remove the pin. This editor only exists in dev mode; it's not part of the deployed site.
...
- **Fallen heroes:** set `status: "fallen"` and add an `epitaph` string on a character in `characters.js` to move them from the active roster grid into the "Fallen Heroes" memorial section instead — desaturated portrait, ribbon badge, epitaph shown on the card and again as "In Memoriam" on their own page. They keep their `/roster/<id>` page either way.
